...
const Prism: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    ...
    const startRAF = () => { if (raf) return; raf = requestAnimationFrame(renderFrame); };
    ...
    const renderFrame = (t) => {
      ...
      if (animationType === "hover") {
        ...
        if (NOISE_ZERO) { const s=Math.abs(yaw-targetYaw)<1e-4&&Math.abs(pitch-targetPitch)<1e-4&&Math.abs(roll)<1e-4; if(s) cont=false; }
      } else if (animationType === "3drotate") {
        const ts=time*TS;
      ...
      if (cont) raf = requestAnimationFrame(renderFrame); else raf = 0;
    };
